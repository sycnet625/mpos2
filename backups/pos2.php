...
// VERSIÓN: 4.6 - FINAL PRODUCTION
ob_start();
session_start();

if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    if (isset($_GET['load_history']) || isset($_GET['load_products'])) {
        ob_clean(); header('Content-Type: application/json'); echo json_encode(['status'=>'error', 'msg'=>'Sesión expirada']); exit;
    }
...
if (isset($_GET['load_history'])) {
    ob_clean(); header('Content-Type: application/json');
    try {
        $idSesion = isset($_GET['session_id']) ? intval($_GET['session_id']) : 0;
        
        $sql = "SELECT id, fecha, total, metodo_pago, cliente_nombre, tipo_servicio, mensajero_nombre as mensajero FROM ventas_cabecera WHERE id_caja = ? ORDER BY id DESC";
        $stmt = $pdo->prepare($sql); $stmt->execute([$idSesion]); $tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $sqlDet = "SELECT d.id, d.id_venta_cabecera, d.nombre_producto as nombre, d.cantidad, d.precio, d.id_producto FROM ventas_detalle d JOIN ventas_cabecera v ON d.id_venta_cabecera = v.id WHERE v.id_caja = ?";
        $stmtDet = $pdo->prepare($sqlDet); $stmtDet->execute([$idSesion]); $detalles = $stmtDet->fetchAll(PDO::FETCH_ASSOC);

        $totalVenta = 0; $totalDev = 0; $countDev = 0;
        foreach($tickets as $t) { $total = floatval($t['total']); if($total < 0) { $totalDev += abs($total); $countDev++; } else { $totalVenta += $total; } }

        echo json_encode(['status' => 'success', 'tickets' => $tickets, 'detalles' => $detalles, 'totales' => ['total' => $totalVenta, 'devoluciones' => $countDev, 'valor_devoluciones' => $totalDev, 'count' => count($tickets)]]);
...
if (isset($_GET['load_products'])) {
    ob_clean(); header('Content-Type: application/json');
    $configFile = 'pos.cfg';
    $config = ["id_almacen" => 1, "id_sucursal" => 1, "mostrar_materias_primas" => false, "mostrar_servicios" => true, "categorias_ocultas" => []];
    ...
    try {
        ...
        if (!empty($config['categorias_ocultas'])) { $placeholders = implode(',', array_fill(0, count($config['categorias_ocultas']), '?')); $cond[] = "p.categoria NOT IN ($placeholders)"; }
        ...
        echo json_encode(['status' => 'success', 'products' => $prods]);
...
// RENDER HTML
$configFile = 'pos.cfg';
$config = [ "tienda_nombre" => "MI TIENDA", "cajeros" => [["nombre"=>"Admin", "pin"=>"0000"]], "id_almacen" => 1, "id_sucursal" => 1 ];
if (file_exists($configFile)) { $loaded = json_decode(file_get_contents($configFile), true); if ($loaded) $config = array_merge($config, $loaded); }
...
$prods = []; $clientsData = []; $mensajeros = []; $cats = [];
try {
    $clientsData = $pdo->query("SELECT id, nombre, telefono, direccion, nit_ci FROM clientes WHERE activo = 1 ORDER BY nombre ASC")->fetchAll(PDO::FETCH_ASSOC);
    try { $mensajeros = $pdo->query("SELECT nombre FROM clientes WHERE activo = 1 AND es_mensajero = 1 ORDER BY nombre ASC")->fetchAll(PDO::FETCH_COLUMN); } catch(Exception $e){}
...
<head>
    ...
    <style>
        ...
        .product-card.disabled-card { opacity: 0.85; filter: grayscale(0.5); } /* Opacidad reducida */
        ...
        <div class="p-2 bg-dark-blue text-white top-bar shadow-sm" style="height: 75px; display: flex; flex-direction: column; justify-content: space-between;">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-1 flex-nowrap"> 
                    ...
                    <span id="netStatus" class="badge bg-secondary" style="font-size: 0.7rem;">
