...
window.renderProducts = function(cat, term) {
    ...
    list.forEach(p => {
        ...
        // AGREGADO: ID único al badge para actualización en tiempo real
        card.innerHTML = `<div id="stock-badge-${p.codigo}" class="stock-badge ${badgeClass}">${stockDisplay}</div>${imgHTML}<div class="product-info"><div class="product-name">${p.nombre}</div><div class="product-price">$${parseFloat(p.precio).toFixed(2)}</div></div>`;
        if (hasStock) { card.onclick = () => addToCart(p); } else { card.onclick = () => { Synth.error(); showToast('Sin Stock', 'error'); }; }
        grid.appendChild(card);
    });
    // Sincronizar badges con el estado actual del carrito tras renderizar el catálogo
    updateStockBadges();
};
...
window.addToCart = function(p) {
    ...
    if(idx >= 0) { 
        if(p.es_servicio==0 && (cart[idx].qty+1) > stockAvailable) { 
            Synth.error(); 
        ...
        if(p.es_servicio==0 && stockAvailable < 1) {
            Synth.error();
            return showToast("Sin stock", "error");
        }
...
window.renderCart = function() {
    ...
    if(cart.length===0) {
        c.innerHTML = '<div class="text-center text-muted mt-5 pt-5"><i class="fas fa-shopping-basket fa-2x mb-2 opacity-25"></i><p class="small">Carrito Vacío</p></div>';
    } else {
        cart.forEach((i, idx) => {
...
// Función para actualizar badges de stock en tiempo real
window.updateStockBadges = function() {
    // Crear un mapa de cantidades en el carrito
    const cartQtys = {};
    cart.forEach(item => {
        cartQtys[item.id] = (cartQtys[item.id] || 0) + item.qty;
    });

    // Actualizar cada card en el DOM
    document.querySelectorAll('.product-card').forEach(card => {
        // Asumiendo que guardamos el id en un data-attribute o lo buscamos por nombre
        // Como renderProducts no ponía data-id, vamos a modificar renderProducts también para que lo ponga
        // Pero mientras, podemos buscar el badge y el id
        // Para que esto sea eficiente, mejor modificamos renderProducts
    });
    
    // Si no queremos modificar renderProducts masivamente, relanzamos el renderProducts 
    // pero eso perdería el scroll del catálogo. 
    // Mejor busquemos los badges directamente si existen.
    productsDB.forEach(p => {
        const inCart = cartQtys[p.codigo] || 0;
        const stockReal = parseFloat(p.stock) || 0;
        const stockVisual = p.es_servicio == 1 ? '∞' : (stockReal - inCart);
        
        // Buscar el elemento de stock por un identificador único (que agregaremos en renderProducts)
        ...
        if(badge) {
            ...
            if(stockVisual <= 0 && p.es_servicio == 0) {
                ...
                badge.closest('.product-card').classList.add('disabled-card');
            } else {
                badge.className = 'stock-badge stock-ok';
                badge.closest('.product-card').classList.remove('disabled-card');
            }
        }
    });
};

window.modifyQty = function(d) { 
...
window.removeItem = function() { 
    if(selectedIndex>=0 && confirm('Eliminar?')) { 
        cart.splice(selectedIndex,1); 
        selectedIndex=-1; 
        Synth.removeCart(); 
        renderCart(); 
...
window.clearCart = function() { 
    if(cart.length>0 && confirm('Vaciar?')) { 
        cart=[]; 
        globalDiscountPct=0; 
        selectedIndex=-1; 
        Synth.clear(); 
