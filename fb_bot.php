              ...
              <div class="card border mb-3">
                ...
                <div class="card-body">
                  ...
                  <div class="d-flex flex-wrap gap-2 mb-2">
                    <button class="btn btn-sm btn-outline-secondary active" id="manualGroupsFilter_all" type="button" onclick="setManualGroupsFilter('all')">Todos</button>
                    <button class="btn btn-sm btn-outline-success" id="manualGroupsFilter_publicable" type="button" onclick="setManualGroupsFilter('publicable')">Publicables</button>
                    <button class="btn btn-sm btn-outline-danger" id="manualGroupsFilter_no_publicable" type="button" onclick="setManualGroupsFilter('no_publicable')">No publicables</button>
                    <button class="btn btn-sm btn-outline-dark" id="manualGroupsFilter_unknown" type="button" onclick="setManualGroupsFilter('unknown')">Sin probar</button>
                  </div>
...
let manualGroupsFilter='all';
function setManualGroupsFilter(mode='all'){manualGroupsFilter=String(mode||'all'); ['all','publicable','no_publicable','unknown'].forEach(key=>{const btn=document.getElementById('manualGroupsFilter_'+key); if(btn) btn.classList.toggle('active', key===manualGroupsFilter);}); renderManualGroups();}
function renderManualGroups(){const wrap=document.getElementById('manualGroupsWrap'); if(!wrap) return; if(!manualGroups.length){wrap.innerHTML='<div class="text-muted small">Sin grupos configurados.</div>'; updateTargetInfo(); return;} const visible=manualGroups.filter(g=>manualGroupsFilter==='all' || String(g.publish_status||'unknown')===manualGroupsFilter); if(!visible.length){wrap.innerHTML='<div class="text-muted small">No hay grupos para ese filtro.</div>'; updateTargetInfo(); return;} wrap.innerHTML=visible.map((g)=>{const idx=manualGroups.indexOf(g); const status=String(g.publish_status||'unknown'); const statusBadge=status==='publicable'?'<span class="badge bg-success ms-1">publicable</span>':(status==='no_publicable'?'<span class="badge bg-danger ms-1">no publicable</span>':'<span class="badge bg-light text-dark ms-1">sin probar</span>'); const lastMsg=String(g.last_test_msg||'').trim(); const lastAt=String(g.last_test_at||'').trim(); return `<div class="border-bottom py-2 small"><div class="d-flex align-items-start gap-2"><div class="flex-grow-1"><div class="fw-semibold">${esc(g.name||('Grupo'))} ${Number(g.enabled||0)===1?'<span class="badge bg-success ms-1">activo</span>':'<span class="badge bg-secondary ms-1">inactivo</span>'}${statusBadge}</div><div class="text-muted">ID: ${esc(g.id||'-')}</div>${g.url?`<a href="${esc(g.url)}" target="_blank" rel="noopener" class="text-decoration-none">${esc(g.url)}</a>`:'<div class="text-muted">Sin URL</div>'}<div class="text-muted">${String(g.access_token||'').trim()!==''?'Token específico guardado':'Usa token principal si existe'}</div>${lastAt?`<div class="text-muted">Última prueba: ${esc(lastAt)}</div>`:''}${lastMsg?`<div class="small ${status==='publicable'?'text-success':'text-danger'}">${esc(lastMsg)}</div>`:''}</div><div class="d-flex gap-1"><button class="btn btn-sm btn-outline-info" type="button" onclick="testGroup(${idx})"><i class="fas fa-vial"></i></button><button class="btn btn-sm btn-outline-${Number(g.enabled||0)===1?'warning':'success'}" type="button" onclick="toggleManualGroup(${idx})"><i class="fas fa-power-off"></i></button><button class="btn btn-sm btn-outline-danger" type="button" onclick="removeManualGroup(${idx})"><i class="fas fa-trash"></i></button></div></div></div>`;}).join(''); updateTargetInfo();}
...
async function testGroup(idx){const gRow=manualGroups[idx]; if(!gRow || !String(gRow.id||'').trim()){a('danger','Grupo inválido'); return;} const d=await p(API+'?action=test_group',{id:String(gRow.id||''),name:String(gRow.name||''),access_token:gRow._token_edit||'',text:`Prueba automática grupo Facebook ${new Date().toISOString()}`}); if(Array.isArray(d.rows)) manualGroups=d.rows; renderManualGroups(); if(d.status==='success' || d.ok===true){a('success','Grupo validado y publicación enviada'); loadRecentPosts(); loadStats();} else a('danger',d.msg||d.error||'No se pudo publicar en el grupo');}
