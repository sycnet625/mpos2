...
$pdo->exec("
    ...
    SELECT
        u.id,
        COALESCE(s.id_empresa, {$defaultEmpresa}) AS id_empresa,
        COALESCE(u.id_sucursal, {$defaultSucursal}) AS id_sucursal,
        COALESCE(
            (
                SELECT a.id
                FROM almacenes a
...
$cashierStmt = $pdo->prepare("
    INSERT IGNORE INTO pos_cashiers
    (nombre, pin, rol, id_empresa, id_sucursal, id_almacen, activo)
    VALUES (?, ?, ?, ?, ?, ?, 1)
");

$cashierStmt->execute(['Admin', '0000', 'admin', $defaultEmpresa, $defaultSucursal, $defaultAlmacen]);

$userCount = (int)$pdo->query("SELECT COUNT(*) FROM pos_user_contexts")->fetchColumn();
$cashierCount = (int)$pdo->query("SELECT COUNT(*) FROM pos_cashiers")->fetchColumn();

echo "OK\n";
echo "pos_user_contexts={$userCount}\n";
echo "pos_cashiers={$cashierCount}\n";
