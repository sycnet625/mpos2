...
(function (ns) {
    ...
    ns.openIntegrationModal = function () {
        ...
        ns.$('integrationModalWrap').innerHTML = '<div class="modal active"><header><h3>🔔 Integraciones RAC</h3><button class="close" data-close-modal="integrationModalWrap">×</button></header>'
            ...
            + ns.integrationField('alertLowBalanceThreshold', 'Alerta saldo bajo (CUP)', s.alertLowBalanceThreshold || 1000, 'number')
    ...
    ns.renderAdmin = function () {
        ...
        if (state.adminTab === 'dashboard') {
            ...
            var pushSummary = state.webpushDeliverySummary || {};
            var pushEvents = (state.webpushDeliveryEvents || []).slice(0, 8);
            var pushSubs = (state.webpushSubscriptions || []).slice(0, 8);
            ...
            body = '<div class="grid kpis">'
                ...
                + ns.kpi('🔔', 'Push activos', String(pushSummary.active || 0), 'Suscripciones válidas', '#ffd700')
                + ns.kpi('📨', 'Push enviados', String(pushSummary.okEvents || 0), 'Eventos entregados', '#28a745')
                + ns.kpi('⚠️', 'Push fallidos', String(pushSummary.failEvents || 0), 'Errores de entrega', '#ef5350')
                + ns.kpi('🚫', 'Endpoints inválidos', String(pushSummary.invalid || 0), 'Suscripciones vencidas', '#ff8c00')
                ...
                }).join('') + '</div></div></div><div class="grid two" style="margin-top:18px"><div class="card"><div class="item-title">Entregas push recientes</div><div class="list" style="margin-top:12px">' + (pushEvents.length ? pushEvents.map(function (evt) { var ctx = evt.context || {}; return '<div class="item"><div class="item-head"><div><div class="item-title">' + ns.esc(evt.eventType) + '</div><div class="sub">' + ns.esc(evt.message) + '</div></div><div>' + ns.badge(evt.eventType === 'webpush_notified' ? 'active' : 'no_sale') + '</div></div><div class="sub" style="margin-top:8px">' + ns.esc((ctx.endpoint || '') + ' · ' + (evt.createdAt || '')) + '</div></div>'; }).join('') : '<div class="item"><div class="sub">Sin entregas push registradas todavía.</div></div>') + '</div></div><div class="card"><div class="item-title">Suscripciones push</div><div class="list" style="margin-top:12px">' + (pushSubs.length ? pushSubs.map(function (sub) { return '<div class="item"><div class="item-head"><div><div class="item-title">' + ns.esc((sub.displayName || sub.username || 'Suscripción') + ' · ' + (sub.role || '')) + '</div><div class="sub">' + ns.esc(sub.endpointShort || '') + '</div></div><div>' + ns.badge(sub.status === 'active' ? 'active' : 'no_sale') + '</div></div><div class="sub" style="margin-top:8px">Último envío ' + ns.esc(sub.lastNotifiedAt || 'N/D') + ' · alta ' + ns.esc(sub.createdAt || '') + '</div></div>'; }).join('') : '<div class="item"><div class="sub">Sin suscripciones push registradas.</div></div>') + '</div></div></div><div class="card" style="margin-top:18px"><div class="item-title">Estado de integraciones</div><div class="list" style="margin-top:12px"><div class="item"><div class="item-head"><div><div class="item-title">Telegram</div><div class="sub">Bot configurado: ' + (state.integrations.telegramConfigured ? 'sí' : 'no') + ' · Gestor: ' + ns.esc((state.integrationSettings.defaultGestorId || '') + ' · ' + (state.integrationSettings.defaultGestorName || '')) + '</div></div><div>' + ns.badge(state.integrations.telegramConfigured ? 'active' : 'pending') + '</div></div><div class="sub" style="margin-top:10px">Chat ID configurado: ' + (state.integrationSettings.defaultGestorChatId ? 'sí' : 'no') + ' · Web push ' + ((state.integrationSettings.webpushEnabled && state.integrationSettings.vapidConfigured) ? 'activo' : 'pendiente') + '</div><div class="actions"><button class="btn ghost" data-open-integrations>⚙️ Configurar Telegram</button></div></div></div></div>';
