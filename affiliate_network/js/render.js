        ...
        if (state.gestorTab === 'marketplace') {
            ...
            body = '<div class="section-title"><h3>Mis enlaces activos</h3></div><div class="list">'
                + (linkPage.items.length ? linkPage.items.map(function (link) {
                    return '<div class="item"><div class="item-head"><div><div class="item-title">' + ns.esc(link.product) + '</div><div class="code" style="margin-top:8px">' + ns.esc(link.link) + '</div><div class="sub" style="margin-top:8px">Ref ' + ns.esc(link.maskedRef) + '</div></div><div style="display:flex;gap:16px;flex-wrap:wrap"><div style="text-align:center"><div class="sub">Clics</div><div class="money">' + ns.esc(link.clicks) + '</div></div><div style="text-align:center"><div class="sub">Leads</div><div class="money">' + ns.esc(link.leads) + '</div></div><div style="text-align:center"><div class="sub">Ganado</div><div class="money">' + ns.formatCUP(link.earned) + '</div></div></div></div><div class="actions"><button class="btn ghost" data-copy-link="' + ns.esc(link.link) + '">📋 Copiar link</button><button class="btn ghost" data-copy-link="' + ns.esc(link.link) + '">📊 Estadisticas</button></div></div>';
