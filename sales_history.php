...
// VERSIÓN: 5.7 (FEAT: INTEGRACIÓN TOTAL DE KPIs GLOBALES)
    ...
    try {
        ...
        foreach ($pagos as $metodo => $monto) {
            if ($monto > 0) {
                ...
                $stmtIns->execute([$asientoID, $fechaContablePost, $cuentaDebe, "Ingreso $metodo - $refCheck", $monto, 0, $_SESSION['admin_name'] ?? 'Admin']);
            }
        }

        if ($totalVenta > 0) {
            $stmtIns->execute([$asientoID, $fechaContablePost, '401.01', "Ventas del Día - $cajero ($refCheck)", 0, $totalVenta, $_SESSION['admin_name'] ?? 'Admin']);
        }
...
// Condiciones de SQL según Alcance
...
try {
    ...
    $sqlSesiones = "SELECT s.*, 
                    ...
                    WHERE s.fecha_contable BETWEEN ? AND ? AND $scopeCondSes
    ...
    if (!empty($sessionIds)) {
        $inQuery = "v.id_sesion_caja IN (" . implode(',', $sessionIds) . ")";
    ...
    $sqlInvInicial = "SELECT SUM(k.cantidad * p.costo) FROM kardex k JOIN productos p ON k.id_producto = p.codigo WHERE k.fecha < ? AND $scopeCondInv";
    $stmtInvIni = $pdo->prepare($sqlInvInicial);
    $stmtInvIni->execute([$start . ' 00:00:00']);
    $valorInv = floatval($stmtInvIni->fetchColumn() ?: 0);

    $sqlInvChg = "SELECT DATE(k.fecha) as dia, SUM(k.cantidad * p.costo) as cambio FROM kardex k JOIN productos p ON k.id_producto = p.codigo WHERE k.fecha BETWEEN ? AND ? AND $scopeCondInv GROUP BY DATE(k.fecha)";
    $stmtInvChg = $pdo->prepare($sqlInvChg);
    $stmtInvChg->execute([$start . ' 00:00:00', $end . ' 23:59:59']);
...
// KPIs Web y Reservas
$sqlWeb = "SELECT COUNT(*) as cant, SUM(total) as total FROM pedidos_cabecera WHERE estado = 'completado' AND DATE(fecha) BETWEEN '$start' AND '$end'";
if($scope === 'local') $sqlWeb .= " AND id_sucursal = $SUC_ID_LOCAL";
$kpiWeb = $pdo->query($sqlWeb)->fetch(PDO::FETCH_ASSOC);

$sqlRes = "SELECT SUM(d.cantidad * (d.precio - p.costo)) as ganancia, SUM(d.cantidad * d.precio) as venta 
           FROM ventas_detalle d 
           JOIN productos p ON d.id_producto = p.codigo 
           JOIN ventas_cabecera v ON d.id_venta_cabecera = v.id 
           LEFT JOIN caja_sesiones s ON v.id_sesion_caja = s.id
            ...
            <div class="row align-items-center">
                <div class="col-md-4">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h5 class="mb-0 fw-bold text-dark"><?php echo htmlspecialchars($s['nombre_cajero'] ?? 'Cajero'); ?></h5>
                    </div>
                    <div class="text-muted small mt-1">Caja #<?php echo $sid; ?> | <span class="text-primary fw-bold">F. Contable: <?php echo $fechaContableStr; ?></span></div>
                </div>
                <div class="col-md-4 text-center">
                    ...
                    <?php elseif(!$isOpen): ?>
                        <form method="POST" onsubmit="return confirm('¿Contabilizar esta sesión?')">
                            ...
                            <button type="submit" class="btn btn-sm btn-outline-primary"><i class="fas fa-book"></i> Contabilizar</button>
                        </form>
                    <?php endif; ?>
                </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover table-sm mb-0 ticket-row">
                    ...
                    <tbody>
                    <?php foreach($sessionTickets as $t): 
                        $safeClient = htmlspecialchars($t['cliente_nombre'], ENT_QUOTES);
                        $safeMsj = isset($t['mensajero_nombre']) ? htmlspecialchars($t['mensajero_nombre'], ENT_QUOTES) : '';
                    ...
                    ?>
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                <button class="btn btn-sm btn-link text-muted p-0" onclick="window.open('ticket_view.php?id=<?php echo $t['id']; ?>', 'Ticket', 'width=400,height=600')"><i class="fas fa-eye"></i></button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
            ...
            <form action="invoice_generator.php" method="POST" target="_blank">
                <div class="modal-body">
                    <div class="row g-2">
                        ...
                        <div class="col-md-12">
                            <label class="form-label small fw-bold">Número Factura Sugerido</label>
...
<script>
    ...
    function printRange() { 
        ...
        const scp = '<?php echo $scope; ?>';
        window.open(`report_print.php?mode=range&start=${start}&end=${end}&scope=${scp}`, 'Reporte', 'width=900,height=800'); 
    }
    
    function printSession(id) { 
        window.open(`report_print.php?mode=session&id=${id}`, 'Reporte', 'width=900,height=800'); 
    }
