...
<script>
    ...
    // 2. Función para determinar si mostrar un producto según el filtro de stock
    window.shouldShowProduct = function(p) {
        if (!window.stockFilterActive) return true; // Si el filtro está desactivado, mostrar todo
        const stock = parseFloat(p.stock) || 0;
        return stock > 0 || p.es_servicio == 1; // Mostrar solo con stock o servicios
    };

    // 3. Función para alternar el filtro de stock
    ...
    // 4. Función para alternar imágenes
    window.toggleImages = function() {
        ...
        if (btn) {
            if (document.body.classList.contains('mode-no-images')) {
                btn.classList.add('btn-filter-active');
                btn.title = 'Imágenes ocultas';
            } else {
                btn.classList.remove('btn-filter-active');
                btn.title = 'Imágenes visibles';
            }
        }
    };
    ...
    window.renderProducts = function(category, searchTerm) {
        // Si no se pasa categoría, intenta mantener la actual
        ...
        if (!grid) {
            console.error('productContainer no encontrado');
            return;
        }
        
        grid.innerHTML = '';
        
        // Usar término de búsqueda pasado o del input
        const searchInput = document.getElementById('searchInput');
        const term = (typeof searchTerm === 'string' ? searchTerm : (searchInput ? searchInput.value : '')).toLowerCase();

        // Usar productsDB (definido en pos.js) o PRODUCTS_DATA (inyectado por PHP)
        const sourceData = window.productsDB || window.PRODUCTS_DATA || [];
        
        console.log(`renderProducts: ${sourceData.length} productos, cat=${category}, term=${term}`);
