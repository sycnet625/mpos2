        ...
        <div class="modal-content">
            ...
            <div class="modal-body pt-4 px-4">
                ...
                <div class="btn-group w-100 mb-4 shadow-sm" role="group">
                    ...
                    <label class="btn btn-outline-warning fw-bold text-dark" for="modeCard"><i class="fas fa-credit-card me-1"></i> Tarjeta</label>
                ...
                <div id="cashPaymentSection"><div class="mb-3"><label class="form-label small fw-bold text-muted text-uppercase">Monto Entregado</label><div class="input-group input-group-lg"><span class="input-group-text bg-success text-white border-success">$</span><input type="number" id="singleAmountInput" class="form-control fw-bold text-end border-success text-success" placeholder="0.00" oninput="calcChange()" style="font-size: 1.5rem;"></div></div><div class="text-end bg-light p-2 rounded border"><span class="text-muted fs-5 me-2">Su Cambio:</span><span class="h2 fw-bold text-primary mb-0" id="singleChangeDisplay">$0.00</span></div></div>
                ...
                <div id="mixedPaymentSection" class="d-none">
                    ...
                    <div class="input-group mb-2"><span class="input-group-text bg-success text-white" style="width: 120px;">Efectivo</span><input type="number" class="form-control split-payment-input text-end" id="payCash" placeholder="0.00" min="0" step="0.01"></div>
                    <div class="input-group mb-2"><span class="input-group-text bg-primary text-white" style="width: 120px;">Transferencia</span><input type="number" class="form-control split-payment-input text-end" id="payTransfer" placeholder="0.00" min="0" step="0.01"></div>
                    <div class="input-group mb-2"><span class="input-group-text bg-warning text-dark" style="width: 120px;">Gasto/Tarj.</span><input type="number" class="form-control split-payment-input text-end" id="payCard" placeholder="0.00" min="0" step="0.01"></div>
                ...
                <div class="row g-2 mb-3">
                    <div class="col-7"><label class="small text-muted fw-bold">Cliente</label><div class="input-group input-group-sm"><select id="cliName" class="form-select" onchange="fillClientData(this)"><option value="">Consumidor Final</option><?php foreach($clientsData as $c): ?><option value="<?php echo htmlspecialchars($c['nombre']); ?>" data-tel="<?php echo htmlspecialchars($c['telefono']); ?>" data-dir="<?php echo htmlspecialchars($c['direccion']); ?>"><?php echo htmlspecialchars($c['nombre']); ?> </option><?php endforeach; ?></select><button class="btn btn-outline-secondary" onclick="openNewClientModal()"><i class="fas fa-plus"></i></button></div></div>
                    <div class="col-5"><label class="small text-muted fw-bold">Servicio</label><select id="serviceType" class="form-select form-select-sm" onchange="toggleServiceOptions()"><option value="mostrador">🍽️ Aquí</option><option value="llevar">🥡 Llevar</option><option value="mensajeria">🛵 Delivery</option><option value="reserva">📅 Reserva</option></select></div>
                </div>
                <div class="mb-3 d-none bg-warning bg-opacity-10 p-2 rounded border border-warning" id="reservationDiv"><h6 class="small fw-bold text-dark mb-2"><i class="fas fa-calendar-alt me-1"></i> Datos de Reserva</h6><div class="row g-2"><div class="col-7"><label class="small text-muted">Fecha y Hora</label><input type="datetime-local" class="form-control form-control-sm border-secondary" id="reservationDate"></div><div class="col-5"><label class="small text-muted">Abono Inicial</label><input type="number" class="form-control form-control-sm fw-bold text-end border-secondary" id="reservationAbono" placeholder="0.00"></div></div></div>
                <div class="mb-3 d-none bg-info bg-opacity-10 p-2 rounded border border-info" id="deliveryDiv"><label class="small fw-bold text-primary mb-1"><i class="fas fa-motorcycle me-1"></i> Asignar Mensajero:</label><select class="form-select form-select-sm border-primary" id="deliveryDriver"><option value="">- Seleccionar -</option><?php foreach($mensajeros as $m): ?><option value="<?php echo htmlspecialchars($m); ?>"><?php echo htmlspecialchars($m); ?></option><?php endforeach; ?></select></div>
...
window.openPaymentModal = function() {
    ...
    const sc = document.getElementById('singleChangeDisplay'); if(sc) sc.innerText='$0.00';
